...
docker compose -p %PROJECT_NAME% %INFRA_FILES% config >nul
if errorlevel 1 (
...
docker compose -p %PROJECT_NAME% %INFRA_FILES% -f docker-compose.dev.yml config >nul
if errorlevel 1 (
...
docker compose -p %PROJECT_NAME% %INFRA_FILES% -f docker-compose.dev.yml up -d
if errorlevel 1 (
...
if /I "%MODE%"=="full" (
  ...
  docker compose -p %PROJECT_NAME% %INFRA_FILES% -f docker-compose.apps.yml -f docker-compose.dev.yml config >nul
  if errorlevel 1 (
    echo [ERROR] Full stack config is invalid.
    echo [HINT] Verify the three docker-compose.infra.* group files exist next to this script.
    exit /b 1
  )

  echo [FULL] Starting full stack...
  docker compose -p %PROJECT_NAME% %INFRA_FILES% -f docker-compose.apps.yml -f docker-compose.dev.yml up -d
  if errorlevel 1 (
...
docker compose -p %PROJECT_NAME% %INFRA_FILES% -f docker-compose.dev.yml ps
