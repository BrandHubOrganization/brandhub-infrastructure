...
* **Biến thể giao diện theo Role (Role-Specific Variations):**
  ...
  * `ROLE_ACCOUNT`: Tập trung hiển thị danh sách các Brand Clients đang phụ trách kèm theo trạng thái tiến độ của các chiến dịch hiện hành và số lượng task đang chờ duyệt chỉnh sửa.
* **Bản đồ Component Chú thích shadcn/ui:**
...
* **Biến thể giao diện theo Role (Role-Specific Variations):**
  ...
  * `ROLE_ACCOUNT`: Chỉ xem được danh sách các Client được Owner/Manager chỉ định và gán quyền quản lý; có quyền thêm mới tài khoản MXH kết nối cho khách hàng đó nhưng không được quyền cấu hình tài nguyên hệ thống hoặc xóa Workspace.
...
* **Biến thể giao diện theo Role (Role-Specific Variations):**
  * `ROLE_ACCOUNT` & `ROLE_CREATOR`: Xem được toàn bộ lịch đăng bài trong không gian làm việc chung. ACCOUNT có quyền thực hiện thao tác kéo thả (Drag-and-Drop) các block bài viết để thay đổi thời gian xuất bản dự kiến, bấm trực tiếp vào ô ngày để lên lịch bài đăng mới.
...
* **Biến thể giao diện theo Role (Role-Specific Variations):**
  * `ROLE_OWNER`, `ROLE_MANAGER` & `ROLE_ACCOUNT`: Tiếp cận toàn bộ dữ liệu phân tích chi tiết của tất cả các tài khoản MXH của khách hàng đã kết nối. Có quyền cấu hình các chỉ số mục tiêu (KPIs) và xuất file dữ liệu thô (.xlsx) hoặc file báo cáo đẹp mắt gửi khách hàng (.pdf).
