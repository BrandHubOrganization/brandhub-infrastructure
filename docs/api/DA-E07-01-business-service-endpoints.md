...
| # | Issue | Fix |
|---|-------|-----|
| 42 | `POST /content-requests` summary table listed `MANAGER+` — missing `CLIENT` | Fixed in [00_conventions.md](endpoints/00_conventions.md) |
| 54 | `GET /analytics/clients/{id}` summary table missing `CLIENT` | Fixed |
| 57 | `GET /reports/{jobId}` summary table missing `CLIENT` | Fixed |
| 37 | `DELETE /posts/{id}` summary table `MANAGER+` implied CREATOR — incorrect | Fixed to `OWNER, MANAGER` |
