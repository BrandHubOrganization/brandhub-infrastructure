...
**Goal:** Build the isolated Client portal, analytics dashboard with charts, automated report generation, and the notification center.
...
- Client can log in to an isolated portal (only sees own data)
...
- CLIENT role enforced by RBAC (Sprint 5)
...
| DA-E38-03 | Implement report email sending (automatically send email to Client on schedule) | Ân (AI) | 🟡 High |
...
|---|---|---|
| Post published | CREATOR, MANAGER | "Post '{title}' published on {platform}" |
| Post failed | MANAGER | "Post '{title}' failed to publish on {platform}" |
| Task assigned | CREATOR | "New task assigned: {topic}" |
| Post submitted for review | MANAGER | "'{title}' is ready for review" |
| Post sent to client | CLIENT | "'{title}' is awaiting your approval" |
| Token expiring in 3 days | MANAGER | "{platform} token expiring in 3 days" |
| Deadline in 24h | CREATOR | "Task '{topic}' deadline in 24 hours" |
...
- [ ] CLIENT logs in, sees only own client data
...
- [ ] Report email sent to Client on schedule
