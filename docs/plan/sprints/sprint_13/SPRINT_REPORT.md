# Sprint 13 Report — Client Portal, Analytics & Notifications

---

## 1. Thông tin Sprint

| Field | Value |
|---|---|
| Sprint | Sprint 13 |
| Timeline | Weeks 25–26 (Nov 4–17, 2026) |
| Phase | Phase 6 — Frontend & Analytics |
| Goal | Isolated Client portal, analytics dashboard with charts, automated report generation, notification center |
| Report date | *(Điền ngày nộp)* |
| Reported by | Lê Trí Trung (Leader) |

---

## 2. Tổng kết hoàn thành

### 2.1 Tỉ lệ hoàn thành theo Epic

| Epic | Tổng tasks | Done | In Review | In Progress | To Do | % Done |
|---|---|---|---|---|---|---|
| E37 — Client Portal | 4 | | | | | |
| E38 — Analytics & Reporting | 4 | | | | | |
| E39 — Notification System | 3 | | | | | |
| **Tổng** | **11** | | | | | |

### 2.2 Tỉ lệ hoàn thành theo thành viên

| Thành viên | Tasks được giao | Done/In Review | Chưa làm | Ghi chú |
|---|---|---|---|---|
| Trung (Leader) | — | — | — | |
| Ân (AI) | — | — | — | |
| Lộc (Frontend) | — | — | — | |

---

## 3. Deliverables đã hoàn thành

| Deliverable | File/Link | Tác giả | Chất lượng |
|---|---|---|---|
| *(Điền)* | | | |

---

## 4. Deliverables chưa hoàn thành

| Task ID | Mô tả | Assignee | Lý do | Kế hoạch |
|---|---|---|---|---|
| *(Điền nếu có)* | | | | |

---

## 5. Đánh giá chất lượng

*(Điền)*

---

## 6. Blocked tasks & Dependencies

*(Điền nếu có)*

---

## 7. Individual highlights

*(Điền)*

---

## 8. Sprint Retrospective

*(Điền)*

---

## 9. Kế hoạch Sprint 14

| Priority | Task | Assignee | Ghi chú |
|---|---|---|---|
| 🔴 Critical | DA-E40-01/02/03/05 Mobile core screens | Lộc | |
| 🔴 Critical | DA-E41-01 FCM client integration | Lộc | |
| 🔴 Critical | DA-E41-02 FCM server-side | Trung | |

---

## 10. Links & References

| Resource | Link |
|---|---|
| Jira Sprint 13 Board | https://letritrung2605.atlassian.net/jira/software/projects/DA/boards |

---

*Deadline nộp: 2026-11-17*
