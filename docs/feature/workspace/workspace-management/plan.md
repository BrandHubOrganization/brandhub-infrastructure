...
- Frontend role type: đổi `UserRole` trong `authStore.ts` sang `MemberRole` (OWNER/MANAGER/CREATOR/CLIENT) — việc sửa `authStore` là prerequisite chung cho cả Workspace pages và RBAC UI-gating, nên làm sớm trong nhánh này.
